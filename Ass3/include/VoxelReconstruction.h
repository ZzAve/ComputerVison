...
public:
	...
	std::vector<int> determine_best_hsv(std::vector<Camera *>,Scene3DRenderer);
	std::vector<int> getBestHsv(Camera *,Scene3DRenderer, cv::Mat,int);
	int compare_images(cv::Mat, cv::Mat);
	void run(int, char**);
};
